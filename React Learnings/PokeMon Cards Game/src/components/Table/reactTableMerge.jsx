...
export function ReactTable({ data, columns }) {
  ...
  return (
    <div className="mx-auto border border-gray-200 rounded-lg overflow-y">
          ...
          {table.getHeaderGroups().map((hg) => (
            <tr key={hg.id} className="border-b border-gray-300">
              {hg.headers.map((header) => (
                <th
                  key={header.id}
                  className="py-3 px-5 text-sm md:text-base font-semibold text-left bg-gray-100"
                >
                  {flexRender(
          ...
          {table.getRowModel().rows.map((row) => (
            <tr key={row.id}>
              {row.getVisibleCells().map((cell) => (
                <td key={cell.id} className="p-2.5 border-b border-gray-200 ">
      ...
      <div className="flex justify-center gap-2 sticky bottom-0 bg-gray-200 p-2">
        ...
        <select
          className="font-semibold text-gray-700 bg-white border-gray-300 rounded-md"
